...
public class WebhookService {
    ...
    public Optional<Execution> newExecution(WebhookContext context, Flow flow, AbstractWebhookTrigger trigger, io.kestra.core.models.tasks.Output output) {
        ...
        // here, whereas an execution's inputs are its resolved, typed values. The execution's are walked to decrypt
        // SECRET inputs, which fails on a value that is not yet the encrypted map — whatever shape it has. They are
        // set further down, once rendered and read through FlowInputOutput, as the other triggers get theirs.
